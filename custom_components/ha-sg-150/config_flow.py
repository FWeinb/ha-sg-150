...
class SG150FlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                ...
                if await self.async_set_unique_id(host_address.lower()):
                    ...
                    password = SG150ApiClient.hash_password(
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                await self.get_user_config_schema(user_input)
            ),
            errors=errors,
        )
    ...
    async def async_step_user_options(
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                await get_options_schema(user_options)
            )
        )
    ...
    async def get_user_config_schema(self, entry: dict | None = None) -> dict:
        ...
        if entry is None:
            entry = {}
        return {
            vol.Required(CONF_HOST_ADDRESS, default=(entry or {}).get(CONF_HOST_ADDRESS, vol.UNDEFINED)): selector.TextSelector(
                selector.TextSelectorConfig(
                    type=selector.TextSelectorType.TEXT
                ),
            ),

            vol.Required(CONF_USERNAME, default=(entry or {}).get(CONF_USERNAME, 'admin')): selector.TextSelector(
                selector.TextSelectorConfig(
                    type=selector.TextSelectorType.TEXT,
                ),
            ),
            vol.Required(CONF_PASSWORD, default=(entry or {}).get(CONF_PASSWORD, vol.UNDEFINED)): selector.TextSelector(
                selector.TextSelectorConfig(
    ...
    async def _test_credentials(self, host_address: str, username: str, password: str) -> None:
...
async def get_options_schema(entry: dict | None = None) -> dict:
    ...
    return {
        vol.Required(CONF_SIP_CALLER_IP, default=(entry or {}).get(CONF_SIP_CALLER_IP, vol.UNDEFINED)): selector.TextSelector(
            selector.TextSelectorConfig(
                type=selector.TextSelectorType.TEXT
            ),
        ),
        vol.Required(CONF_SIP_USER, default=(entry or {}).get(CONF_SIP_USER, vol.UNDEFINED)): selector.TextSelector(
            selector.TextSelectorConfig(
                type=selector.TextSelectorType.TEXT,
            ),
        ),
        vol.Required(CONF_SIP_PASSWORD, default=(entry or {}).get(CONF_SIP_PASSWORD, vol.UNDEFINED)): selector.TextSelector(
            selector.TextSelectorConfig(
                type=selector.TextSelectorType.PASSWORD,
            ),
        ),
        vol.Required(CONF_SIP_NUMBER, default=(entry or {}).get(CONF_SIP_NUMBER, vol.UNDEFINED)): selector.TextSelector(
            selector.TextSelectorConfig(
                type=selector.TextSelectorType.TEXT,
            ),
        ),
        vol.Required(CONF_SIP_PORT, default=(entry or {}).get(CONF_SIP_PORT, vol.UNDEFINED)): selector.TextSelector(
            selector.TextSelectorConfig(
